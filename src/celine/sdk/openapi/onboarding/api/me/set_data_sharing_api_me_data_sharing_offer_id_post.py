from http import HTTPStatus
from typing import Any
from urllib.parse import quote

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.data_sharing_decision_request import DataSharingDecisionRequest
from ...models.data_sharing_status_response import DataSharingStatusResponse
from ...models.http_validation_error import HTTPValidationError
from ...types import Response


def _get_kwargs(
    offer_id: str,
    *,
    body: DataSharingDecisionRequest,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/api/me/data-sharing/{offer_id}".format(
            offer_id=quote(str(offer_id), safe=""),
        ),
    }

    _kwargs["json"] = body.to_dict()

    headers["Content-Type"] = "application/json"

    _kwargs["headers"] = headers
    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> DataSharingStatusResponse | HTTPValidationError | None:
    if response.status_code == 200:
        response_200 = DataSharingStatusResponse.from_dict(response.json())

        return response_200

    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())

        return response_422

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[DataSharingStatusResponse | HTTPValidationError]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    offer_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: DataSharingDecisionRequest,
) -> Response[DataSharingStatusResponse | HTTPValidationError]:
    """Set Data Sharing

     Grant or withdraw one offer.

    Withdrawal is the reason this route exists: the onboarding wizard can only
    grant, so without it a consent could be given and never taken back — which
    Art. 7(3) requires to be as easy as giving it.

    Args:
        offer_id (str):
        body (DataSharingDecisionRequest): Grant or withdraw one offer.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[DataSharingStatusResponse | HTTPValidationError]
    """

    kwargs = _get_kwargs(
        offer_id=offer_id,
        body=body,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    offer_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: DataSharingDecisionRequest,
) -> DataSharingStatusResponse | HTTPValidationError | None:
    """Set Data Sharing

     Grant or withdraw one offer.

    Withdrawal is the reason this route exists: the onboarding wizard can only
    grant, so without it a consent could be given and never taken back — which
    Art. 7(3) requires to be as easy as giving it.

    Args:
        offer_id (str):
        body (DataSharingDecisionRequest): Grant or withdraw one offer.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        DataSharingStatusResponse | HTTPValidationError
    """

    return sync_detailed(
        offer_id=offer_id,
        client=client,
        body=body,
    ).parsed


async def asyncio_detailed(
    offer_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: DataSharingDecisionRequest,
) -> Response[DataSharingStatusResponse | HTTPValidationError]:
    """Set Data Sharing

     Grant or withdraw one offer.

    Withdrawal is the reason this route exists: the onboarding wizard can only
    grant, so without it a consent could be given and never taken back — which
    Art. 7(3) requires to be as easy as giving it.

    Args:
        offer_id (str):
        body (DataSharingDecisionRequest): Grant or withdraw one offer.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[DataSharingStatusResponse | HTTPValidationError]
    """

    kwargs = _get_kwargs(
        offer_id=offer_id,
        body=body,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    offer_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: DataSharingDecisionRequest,
) -> DataSharingStatusResponse | HTTPValidationError | None:
    """Set Data Sharing

     Grant or withdraw one offer.

    Withdrawal is the reason this route exists: the onboarding wizard can only
    grant, so without it a consent could be given and never taken back — which
    Art. 7(3) requires to be as easy as giving it.

    Args:
        offer_id (str):
        body (DataSharingDecisionRequest): Grant or withdraw one offer.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        DataSharingStatusResponse | HTTPValidationError
    """

    return (
        await asyncio_detailed(
            offer_id=offer_id,
            client=client,
            body=body,
        )
    ).parsed
